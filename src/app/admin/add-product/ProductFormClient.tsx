...
export default function ProductFormClient() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {error && (
        <div className="p-4 bg-rose-50 border border-rose-200 text-sm font-bold text-rose-700 rounded-xl">
          ⚠️ {error}
        </div>
      )}
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0077B6]/25 focus:border-[#0077B6] font-semibold transition"
        />
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0077B6]/25 focus:border-[#0077B6] font-medium transition resize-none"
        />
      </div>
      ...
      {hasLiveDiscount && (
        <div className="p-4 bg-emerald-50 border border-emerald-250 rounded-2xl flex items-baseline gap-3">
          ...
          <span className="text-sm font-black text-emerald-755">
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          ...
          <button
            ...
            className="text-xs font-extrabold text-[#0077B6] hover:text-[#005f91] transition flex items-center gap-1 cursor-pointer"
          >
            ➕ Add More Images
          </button>
        </div>
        ...
        <div className="space-y-4">
          {imageUrls.map((url, idx) => (
            <div key={idx} className="flex flex-col gap-2 p-3 bg-slate-50 border border-slate-200 rounded-2xl shadow-xs">
              <div className="flex gap-2 items-center">
                <input
                  ...
                  className="flex-1 px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0077B6]/25 focus:border-[#0077B6] font-semibold transition"
                />
                <button
                  type="button"
                  onClick={() => handleRemoveImageField(idx)}
                  className="p-3 bg-white hover:bg-rose-50 text-slate-450 hover:text-rose-600 border border-slate-200 rounded-xl transition text-sm font-bold shadow-xs cursor-pointer"
                >
                  ✕
                </button>
              </div>
              <div className="flex items-center gap-3">
                <label className="relative flex items-center justify-center px-4 py-2 border border-slate-200 rounded-xl bg-white hover:bg-slate-50 hover:text-slate-900 text-xs font-bold text-slate-650 shadow-xs cursor-pointer transition">
                  {uploadingIndex === idx ? (
                    <span className="flex items-center gap-1.5">
                      <svg className="animate-spin h-3.5 w-3.5 text-[#0077B6]" fill="none" viewBox="0 0 24 24">
      ...
      <div className="pt-2">
        <button
          type="submit"
          disabled={loading}
          className="w-full py-4 text-sm font-bold text-white bg-[#FF6B35] hover:bg-orange-600 rounded-xl shadow-md transition disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
        >
