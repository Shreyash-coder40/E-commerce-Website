...
export default function AiAssistantConsolePage() {
  ...
  // Verify authorization & fetch stats on load
  useEffect(() => {
  ...
  const checkAuthAndFetchStats = async () => {
    try {
      // Fetching addresses checks standard authentication
      const authRes = await fetch("/api/addresses");
      if (authRes.status === 401) {
        router.push("/login?callback=/admin/ai-assistant");
        return;
      }

      // Check if user is an admin by querying the chat api with a test payload or fetching addresses
      if (!authRes.ok) {
        router.push("/");
        return;
      }
      ...
      // Fetch basic dashboard statistics for the sidebar console
      const statsRes = await fetch("/api/admin/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: "Show stats summary local trigger", history: [] })
      });
      
      if (statsRes.ok) {
        // Query database summary to populate console panel
        ...
        if (prodRes.ok && prodData.success) {
          ...
          setStats({
            ...
            lowStockCount: low,
            outOfStockCount: out,
            revenue: 0, // Will be computed or spoken by chat
            ordersCount: 0,
            returnLosses: 0
          });
        }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm font-bold text-gray-550">Authenticating Agent Console Permissions...</p>
        </div>
      </div>
    );
  }
