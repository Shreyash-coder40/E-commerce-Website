      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 sm:gap-3">
          ...
          <Link href="/" className="text-base sm:text-xl font-black text-navbar-text tracking-tight hover:text-indigo-600 dark:hover:text-indigo-400 transition flex items-center gap-1 sm:gap-2">
            ...
            <span>NEXT</span><span className="text-indigo-600 dark:text-indigo-400">SHOP</span>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-3 sm:gap-6">
          ...
          {session?.user ? (
            <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
              ...
              <Link
                href="/orders"
                className="text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 hover:bg-indigo-100 dark:hover:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-900/50 px-4 py-2 rounded-xl transition hidden sm:inline-block"
              >
                🛍️ My Orders
              </Link>
              <span className="text-xs font-bold text-card-text-secondary hidden lg:inline">
                Hi, <span className="text-card-text-primary font-extrabold">{session.user.name || "User"}</span>
              </span>
              <button
                onClick={() => signOut({ callbackUrl: "/" })}
                className="text-xs font-bold text-rose-500 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-950/60 border border-rose-200 dark:border-rose-900/50 px-3.5 py-2 rounded-xl transition cursor-pointer"
              >
                Sign Out
              </button>
            </div>
