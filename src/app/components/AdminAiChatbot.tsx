  ...
  const parseMarkdown = (markdown: string) => {
    ...
    lines.forEach((line, idx) => {
      ...
      // Bullet list items
        ...
        elements.push(
          <li key={idx} className="text-xs text-slate-700 ml-4 list-disc mb-1 leading-relaxed">
      ...
      // Numbered list items
        ...
        elements.push(
          <li key={idx} className="text-xs text-slate-700 ml-4 list-decimal mb-1 leading-relaxed">
      ...
      // Paragraph spacing
        ...
        elements.push(<p key={idx} className="text-xs text-slate-700 leading-relaxed mb-2.5 font-medium">{formatInline(line)}</p>);
      }
    });
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className="h-14 w-14 rounded-full bg-[#0077B6] text-white shadow-xl hover:shadow-indigo-500/35 hover:scale-105 active:scale-95 transition flex items-center justify-center cursor-pointer relative"
        >
          {isOpen ? (
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-[420px] max-w-[calc(100vw-3rem)] h-[600px] bg-white/95 backdrop-blur-xl border border-slate-200 rounded-3xl shadow-2xl z-50 flex flex-col overflow-hidden text-slate-800 animate-slide-in font-['Plus_Jakarta_Sans',sans-serif]">
          {/* Header */}
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-white border border-slate-200 rounded-2xl p-4 rounded-bl-none shadow-xs flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-slate-500 font-bold">AI is parsing inventory...</span>
                </div>
              </div>
            )}
          ...
          {attachedPdf && (
            <div className="px-4 py-2 bg-slate-50 border-t border-slate-200 flex items-center justify-between text-xs text-emerald-600 font-semibold gap-2 animate-slide-in">
              ...
              <button 
                ...
                className="text-slate-400 hover:text-slate-750 font-black cursor-pointer px-1.5 py-0.5 rounded hover:bg-slate-200 transition"
              >
                ✕
              </button>
            </div>
          ...
          <form onSubmit={handleFormSubmit} className="p-4 border-t border-slate-200 bg-slate-50 flex items-center gap-2 backdrop-blur-xs">
            ...
            <input
              ...
              placeholder={attachedPdf ? "Ask something about the attached PDF..." : "Ask for stock warnings, profit loss or to add products..."}
