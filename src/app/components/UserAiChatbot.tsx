    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-20 left-6 w-96 max-w-[calc(100vw-3rem)] h-[460px] max-h-[calc(100vh-6rem)] bg-slate-900/90 backdrop-blur-xl border border-slate-800/80 rounded-3xl shadow-2xl z-55 flex flex-col overflow-hidden text-white animate-slide-in font-['Plus_Jakarta_Sans',sans-serif] tracking-tight">
              ...
              >
                <div
                  className={`max-w-[85%] rounded-2xl p-4 shadow-sm text-xs ${
                    msg.sender === "user"
                      ? "bg-indigo-700 text-white font-bold border border-indigo-600 rounded-bl-none"
                      : "bg-slate-950/60 border border-slate-800 text-slate-200 rounded-br-none"
                  }`}
                >
          ...
          {/* Footer form box */}
          <form onSubmit={handleFormSubmit} className="p-4 border-t border-slate-800 bg-slate-950/60 flex items-center gap-2 backdrop-blur-xs">
            <input 
              type="file"
              ref={fileInputRef}
            ...
            <button
              ...
              className="h-9 w-9 rounded-xl bg-slate-900 border border-slate-800 hover:border-slate-700 hover:bg-slate-850 text-slate-400 hover:text-white transition flex items-center justify-center cursor-pointer disabled:opacity-40 flex-shrink-0"
            >
              📎
            </button>
            ...
            <button
              ...
              className="bg-indigo-650 hover:bg-indigo-500 text-white px-4 py-2.5 rounded-xl text-xs font-bold shadow-md transition cursor-pointer disabled:opacity-40 flex-shrink-0"
            >
              Send
            </button>
          </form>
